...
def disp_error_image_func(D_est_tensor, D_gt_tensor, abs_thres=3., rel_thres=0.05, dilate_radius=1):
    # D_gt_np = D_gt_tensor.detach().cpu().numpy()
    # D_est_np = D_est_tensor.detach().cpu().numpy()
    # print("desttensor", len(D_est_tensor))
    D_gt_np = np.array(torch.tensor(D_gt_tensor, device="cpu"))

    # print(D_est_tensor)
    D_est_np = torch.tensor(D_est_tensor, device="cpu")
    D_est_np = np.array(D_est_np)
